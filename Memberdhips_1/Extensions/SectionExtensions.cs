...
namespace Memberdhips_1.Extensions
{
    public static class SectionExtensions
    {
        public static async Task<ProductSectionModel> GetProductSectionAsync(int productId,string userId)
        {
            ...
            var result = sections.Distinct(new ProductSectionEqualityCompare()).ToList();
            ...
            )
        {
            ...
            var items = await (from i in db.Items
                               ...
                               join ps in db.SubscriptionProducts on pi.ProductId equals ps.ProductId
                               join us in db.UserSubscriptions on ps.SubscriptionId equals us.SubscriptionId
                               ...
                               select new ProductItemRow
                               {
                                   ...
                                   ReleaseDate = DbFunctions.CreateDateTime(us.StartDate.Value.Year,
                                          us.StartDate.Value.Month, us.StartDate.Value.Day + i.WaitDay, 0, 0, 0),
