...
namespace Memberdhips_1.Areas.Admin.Extensions
{
    public static class ConversionExtensions
    {
        ...
        this ProductItem productItem, ApplicationDbContext db,
           bool addDataList = true)
        {
            var model = new ProductItemModel
            {
                ItemId = productItem.ItemId,
                ProductId = productItem.ProductId,
                Items = addDataList ? await db.Items.ToListAsync() : null,
                Products = addDataList ? await db.Products.ToListAsync() : null,
                ItemTitle = (await db.Items.FirstOrDefaultAsync(
                    i => i.Id.Equals(productItem.ItemId)
                    )).Title,
                ProductTitle = (await db.Products.FirstOrDefaultAsync(
                    p => p.Id.Equals(productItem.ProductId)
                    )).Title
           ...
           bool addDataList = true)
        {
            var model = new SubscriptionProductModel
            {
                ...
                Subscriptions = addDataList ? await db.Subscriptions.ToListAsync() : null,
                Products = addDataList ? await db.Products.ToListAsync() : null,
                SubscriptionTitle = (await db.Subscriptions.FirstOrDefaultAsync(
                    s => s.Id.Equals(subscriptionProduct.SubscriptionId)
                    )).Title,
                ProductTitle = (await db.Products.FirstOrDefaultAsync(
                    p => p.Id.Equals(subscriptionProduct.ProductId)
                    )).Title
